...
def real_awgn(x, stddev):
    ...
    awgn = torch.randn(x.shape, dtype=torch.float32) * stddev
    y = x + awgn
...
def fading(x, stddev, h=None):
    ...
    if h is None:
        h = torch.complex(
            torch.randn([x.shape[0], 1], dtype=torch.float32) * 1 / np.sqrt(2),
            torch.randn([x.shape[0], 1], dtype=torch.float32) * 1 / np.sqrt(2),
        )
    # additive white gaussian noise
    awgn = torch.complex(
        torch.randn(x.shape, dtype=torch.float32) * 1 / np.sqrt(2),
        torch.randn(x.shape, dtype=torch.float32) * 1 / np.sqrt(2),
    )
...
def phase_invariant_fading(x, stddev, h=None):
    ...
    # channel gain
    if h is None:
        n1 = torch.randn((x.shape[0],1), dtype=torch.float32) * 1 / np.sqrt(2)
        n2 = torch.randn((x.shape[0],1), dtype=torch.float32) * 1 / np.sqrt(2)
        h = torch.sqrt(torch.square(n1) + torch.square(n2))

    # additive white gaussian noise
    awgn = torch.randn(x.shape, dtype=torch.float32) * stddev / np.sqrt(2)
...
class Channel():
    ...
    def __call__(self, inputs):
        ...
        if self.channel_type == "awgn":
            ...
            z_in = z_in * torch.complex(torch.sqrt(torch.tensor(dim_z, dtype=torch.float32) / z_norm), 0.0)
